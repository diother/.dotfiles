...
local function move_thorium_to_tag(c)
    ...
    -- check if the client's class name is in the mapping
    local target_tag_index = class_to_tag_mapping[class_name]
    if target_tag_index then
        -- get the tag object corresponding to the class name
        local target_tag = awful.screen.focused().tags[target_tag_index]
        
        -- move the client to the corresponding tag
        c:move_to_tag(target_tag)
    end
end

-- connect a signal handler to listen for new clients being created
client.connect_signal("manage", move_thorium_to_tag)

-- Spawn each instance of Thorium Browser with the corresponding URL
